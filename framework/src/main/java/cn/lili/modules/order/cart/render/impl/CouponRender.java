...
public class CouponRender implements CartRenderStep {
    ...
    private List<CartSkuVO> filterSkuVo(List<CartSkuVO> cartSkuVOS, MemberCoupon memberCoupon) {
        ...
        switch (PromotionsScopeTypeEnum.valueOf(memberCoupon.getScopeType())) {
            ...
            case PORTION_GOODS:
                //按照商品过滤
                Set<String> scopeGoodsIdSet = new HashSet<>(Arrays.asList(memberCoupon.getScopeId().split(",")));
                filterSku = filterSku.stream().filter(cartSkuVO -> scopeGoodsIdSet.contains(cartSkuVO.getGoodsSku().getId())).collect(Collectors.toList());
                break;
            ...
            case PORTION_GOODS_CATEGORY:
                ...
                //按照店铺分类过滤
                Set<String> scopeCategoryIdSet = new HashSet<>(Arrays.asList(memberCoupon.getScopeId().split(",")));
                filterSku = filterSku.stream().filter(cartSkuVO -> {
                    ...
                    return scopeCategoryIdSet.contains(categoryId);
                }).collect(Collectors.toList());
                break;
            default:
                return Collections.emptyList();
