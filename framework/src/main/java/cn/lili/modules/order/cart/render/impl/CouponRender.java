...
public class CouponRender implements CartRenderStep {
    ...
    private List<CartSkuVO> filterSkuVo(List<CartSkuVO> cartSkuVOS, MemberCoupon memberCoupon) {
        ...
        switch (PromotionsScopeTypeEnum.valueOf(memberCoupon.getScopeType())) {
            ...
            case PORTION_GOODS:
                //按照商品过滤
                Set<String> scopeGoodsSet = new HashSet<>(Arrays.asList(memberCoupon.getScopeId().split(",")));
                filterSku = filterSku.stream().filter(cartSkuVO -> scopeGoodsSet.contains(cartSkuVO.getGoodsSku().getId())).collect(Collectors.toList());
                break;
            ...
            case PORTION_GOODS_CATEGORY:
                ...
                //按照店铺分类过滤
                Set<String> scopeCategorySet = new HashSet<>(Arrays.asList(memberCoupon.getScopeId().split(",")));
                filterSku = filterSku.stream().filter(cartSkuVO -> {
                    ...
                    return scopeCategorySet.contains(categoryId);
                }).collect(Collectors.toList());
                break;
            default:
                return Collections.emptyList();
    ...
    private List<CartSkuVO> filterPromotionShopCategory(List<CartSkuVO> filterSku, MemberCoupon memberCoupon) {
        Set<String> scopeShopCategorySet = new HashSet<>(Arrays.asList(memberCoupon.getScopeId().split(",")));
        return filterSku.stream().filter(cartSkuVO -> {
            if (CharSequenceUtil.isNotEmpty(cartSkuVO.getGoodsSku().getStoreCategoryPath())) {
                ...
                for (String category : storeCategoryPath) {
                    ...
                    if (scopeShopCategorySet.contains(category)) {
                        return true;
                    }
                }
            }
