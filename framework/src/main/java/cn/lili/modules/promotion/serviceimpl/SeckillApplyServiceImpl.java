...
public class SeckillApplyServiceImpl extends ServiceImpl<SeckillApplyMapper, SeckillApply> implements SeckillApplyService {
    ...
    public boolean updateSeckillApplyTime(Seckill seckill) {
        ...
        Set<String> validHours = new HashSet<>(Arrays.asList(seckill.getHours().split(",")));
        List<SeckillApply> list = this.list(queryWrapper).stream().filter(i -> i.getTimeLine() != null && validHours.contains(i.getTimeLine().toString())).collect(Collectors.toList());
