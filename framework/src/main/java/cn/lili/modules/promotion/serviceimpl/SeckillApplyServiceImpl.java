...
public class SeckillApplyServiceImpl extends ServiceImpl<SeckillApplyMapper, SeckillApply> implements SeckillApplyService {
    ...
    public boolean updateSeckillApplyTime(Seckill seckill) {
        ...
        // Optimize: Convert delimited hours string to a Set to achieve O(1) lookups in the stream filter
        String[] seckillHours = seckill.getHours().split(",");
        Set<String> seckillHoursSet = new HashSet<>(Arrays.asList(seckillHours));
    ...
    private void checkSeckillApplyList(String hours, List<SeckillApplyVO> seckillApplyList) {
        // Optimize: Hoist string splitting and stream creation out of loop, convert to Set for O(1) lookups
        String[] rangeHours = hours.split(",");
        Set<String> rangeHoursSet = new HashSet<>(Arrays.asList(rangeHours));
