...
public class SeckillApplyServiceImpl extends ServiceImpl<SeckillApplyMapper, SeckillApply> implements SeckillApplyService {
    ...
    private void checkSeckillApplyList(String hours, List<SeckillApplyVO> seckillApplyList) {
        // ⚡ Bolt: Use Set for O(1) lookups instead of List.contains() to avoid O(N^2) overhead
        Set<String> existSku = new HashSet<>();
        // ⚡ Bolt: Hoist string splitting and use Set for O(1) lookups to avoid O(N) overhead inside loop
        Set<String> validHours = new HashSet<>(Arrays.asList(hours.split(",")));
        for (SeckillApplyVO seckillApply : seckillApplyList) {
            ...
            if (!validHours.contains(seckillApply.getTimeLine().toString())) {
                throw new ServiceException(ResultCode.SECKILL_TIME_ERROR);
            }
            //检查商品是否参加多个时间段的活动
            // ⚡ Bolt: Set.add() returns false if element already exists, performing existence check and insertion safely in a single O(1) operation
