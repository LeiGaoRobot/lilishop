...
public class SeckillApplyServiceImpl extends ServiceImpl<SeckillApplyMapper, SeckillApply> implements SeckillApplyService {
    ...
    public boolean updateSeckillApplyTime(Seckill seckill) {
        ...
        // Optimize: Convert seckill.getHours() (a comma-separated string) to a Set for O(1) lookup
        // Using CharSequenceUtil.contains() or String.contains() on delimited strings can lead to
        // false positive substring matches (e.g., "10" matches "1") and O(N) complexity during streaming.
        Set<String> hoursSet = new java.util.HashSet<>(java.util.Arrays.asList(seckill.getHours().split(",")));

        List<SeckillApply> list = this.list(queryWrapper).stream()
                .filter(i -> i.getTimeLine() != null && hoursSet.contains(i.getTimeLine().toString()))
