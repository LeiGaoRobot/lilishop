...
public class SeckillApplyServiceImpl extends ServiceImpl<SeckillApplyMapper, SeckillApply> implements SeckillApplyService {
    ...
    private void checkSeckillApplyList(String hours, List<SeckillApplyVO> seckillApplyList) {
        // ⚡ Bolt: Use Set instead of List to optimize lookup, and use .add() to perform existence check and insertion in one O(1) operation
        Set<String> existSku = new HashSet<>();
        // ⚡ Bolt: Hoist string splitting and Set conversion outside loop to avoid redundant O(N) parsing and array allocations per iteration
