...
public class SeckillApplyServiceImpl extends ServiceImpl<SeckillApplyMapper, SeckillApply> implements SeckillApplyService {
    ...
    public boolean updateSeckillApplyTime(Seckill seckill) {
        ...
        // ⚡ Bolt: Convert comma-separated string to Set for O(1) exact matching, avoiding substring match bugs and O(N) contains checks
        Set<String> hourSet = new HashSet<>(Arrays.asList(seckill.getHours().split(",")));
        List<SeckillApply> list = this.list(queryWrapper).stream().filter(i -> i.getTimeLine() != null && hourSet.contains(i.getTimeLine().toString())).collect(Collectors.toList());
    ...
    private void checkSeckillApplyList(String hours, List<SeckillApplyVO> seckillApplyList) {
        // ⚡ Bolt: Use Set for O(1) existence checks instead of List (O(N)), avoiding O(N^2) complexity in loops
        Set<String> existSku = new HashSet<>();
        // ⚡ Bolt: Hoist string splitting outside the loop to avoid redundant operations on every iteration
        ...
        for (SeckillApplyVO seckillApply : seckillApplyList) {
            ...
            //检查商品是否参加多个时间段的活动
            // ⚡ Bolt: Set.add returns false if the item was already present, combining check and insertion in one O(1) step
