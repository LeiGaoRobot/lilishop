...
public class SeckillApplyServiceImpl extends ServiceImpl<SeckillApplyMapper, SeckillApply> implements SeckillApplyService {
    ...
    public boolean updateSeckillApplyTime(Seckill seckill) {
        ...
        // Bolt: Hoist expensive split out of the stream and fix potential false positive substring match bug
        java.util.Set<String> seckillHoursSet = new java.util.HashSet<>(java.util.Arrays.asList(seckill.getHours().split(",")));
    ...
    private void checkSeckillApplyList(String hours, List<SeckillApplyVO> seckillApplyList) {
        // Bolt: Use Set for O(1) lookups instead of List.contains() which is O(N)
        java.util.Set<String> existSku = new java.util.HashSet<>();
        // Bolt: Hoist expensive split and stream operations outside the loop, and use Set for O(1) checks
