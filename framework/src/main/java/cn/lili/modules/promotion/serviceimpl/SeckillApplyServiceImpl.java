...
public class SeckillApplyServiceImpl extends ServiceImpl<SeckillApplyMapper, SeckillApply> implements SeckillApplyService {
    ...
    public boolean updateSeckillApplyTime(Seckill seckill) {
        ...
        // ⚡ Bolt: Hoisted string split to prevent O(N) reallocation and replaced String.contains() with Set.contains() to achieve O(1) lookup and prevent substring matching bugs.
    ...
    private void checkSeckillApplyList(String hours, List<SeckillApplyVO> seckillApplyList) {
        // ⚡ Bolt: Replaced List with Set to prevent O(N^2) complexity on existSku.contains()
        Set<String> existSku = new HashSet<>();
        // ⚡ Bolt: Hoisted string split outside the loop to avoid redundant allocation and parsing
