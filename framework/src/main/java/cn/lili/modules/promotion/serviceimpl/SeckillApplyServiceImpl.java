...
public class SeckillApplyServiceImpl extends ServiceImpl<SeckillApplyMapper, SeckillApply> implements SeckillApplyService {
    ...
    public boolean updateSeckillApplyTime(Seckill seckill) {
        ...
        // ⚡ Bolt: Convert delimited string to Set outside stream for O(1) lookup and exact match, preventing substring bugs
    ...
    private void checkSeckillApplyList(String hours, List<SeckillApplyVO> seckillApplyList) {
        // ⚡ Bolt: Hoist string split and convert to Set for O(1) lookup instead of O(N) inside loop
        Set<String> rangeHoursSet = new HashSet<>(Arrays.asList(hours.split(",")));
        // ⚡ Bolt: Use Set for existSku for O(1) lookup instead of O(N) List.contains
