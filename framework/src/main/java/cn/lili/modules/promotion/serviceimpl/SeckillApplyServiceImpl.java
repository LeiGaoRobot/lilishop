...
public class SeckillApplyServiceImpl extends ServiceImpl<SeckillApplyMapper, SeckillApply> implements SeckillApplyService {
    ...
    public boolean updateSeckillApplyTime(Seckill seckill) {
        ...
        // ⚡ Bolt: Convert delimited string to List once to avoid both incorrect substring matches and repeated string operations
        List<String> hoursList = Arrays.asList(seckill.getHours().split(","));
        List<SeckillApply> list = this.list(queryWrapper).stream().filter(i -> i.getTimeLine() != null && hoursList.contains(i.getTimeLine().toString())).collect(Collectors.toList());
    ...
    private void checkSeckillApplyList(String hours, List<SeckillApplyVO> seckillApplyList) {
        // ⚡ Bolt: Use Set for O(1) lookup and prevent O(N) add check overhead
        Set<String> existSku = new HashSet<>();
        // ⚡ Bolt: Hoist string split outside the loop to avoid O(N) redundant splits and array creations
        ...
        for (SeckillApplyVO seckillApply : seckillApplyList) {
            ...
            if (!rangeHoursList.contains(seckillApply.getTimeLine().toString())) {
