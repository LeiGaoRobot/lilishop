...
public class SeckillApplyServiceImpl extends ServiceImpl<SeckillApplyMapper, SeckillApply> implements SeckillApplyService {
    ...
    public boolean updateSeckillApplyTime(Seckill seckill) {
        ...
        // ⚡ Bolt: Convert comma-separated hours string to Set for O(1) lookups instead of O(N) String.contains in filter
        Set<String> seckillHoursSet = seckill.getHours() != null
                ? new HashSet<>(Arrays.asList(seckill.getHours().split(",")))
                : new HashSet<>();
